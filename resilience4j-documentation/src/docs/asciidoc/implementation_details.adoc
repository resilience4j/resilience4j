...
Then, all access to the backend is blocked for a (configurable) time duration. `CircuitBreaker::obtainPermission()` throws a `CircuitBreakerOpenException`, if the CircuitBreaker is `OPEN`.
