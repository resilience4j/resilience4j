...
The CacheContext emits a stream of CacheEvents to any Observer/Consumer who subscribes. An event can be a cache hit, a cache miss or an error. This library provides a consumer `CacheEventConsumer` which can be used to store events in a circular buffer with a fixed capacity.

[source,java]
----
Cache<String, String> cacheContext = Cache.of(cacheInstance);
CacheEventConsumer<CacheOnMissEvent> ringBuffer = new CacheEventConsumer<>(10);
