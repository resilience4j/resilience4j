...
The following example shows how to decorate a method reference and retrieve cached results using JCache.
If the cache contains a value, the value is returned from the cache, otherwise the method reference is called in order to retrieve a value and put it into the cache.
If the cache retrieval from a distributed cache fails, the exception is taken care of and the method reference is called.

[source,java]
----
// Create a CacheContext by wrapping a JCache instance.
Cache<String, String> cacheContext = Cache.of(Caching.getCache("cacheName", String.class, String.class));

// Decorate your call to BackendService.doSomething()
Try.CheckedFunction<String, String> cachedFunction = Cache
    .decorateCheckedSupplier(cacheContext, backendService::doSomething);
