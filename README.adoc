...
The default event listener just logs the state transitions events. If you want to add your custom state transition listener,
you have to implement the functional interface `CircuitBreakerEventListener` which has a method `onCircuitBreakerEvent(CircuitBreakerEvent circuitBreakerEvent)`. Currently there is only the event `CircuitBreakerStateTransitionEvent`.
