...
The project provides several higher-order functions to decorate any `Supplier` or `Runnable` function with a Circuit Breaker. You can use the returned, decorated function synchronously or asynchronously. The project should be combined with a functional library for Java 8 like https://github.com/javaslang/javaslang[javaslang]. The project requires at least JDK 8.
