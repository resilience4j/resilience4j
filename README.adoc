...
The CircuitBreaker is implemented via a (timed) state machine and has three states: `CLOSED`, `OPEN` and `HALF_OPEN`. The CircuitBreaker does not know anything about the backend's state by itself, but uses only the information provided by calls to `CircuitBreaker::recordSuccess()` and `CircuitBreaker::recordFailure()`.
...
[source,java]
----
// Create a InMemoryCircuitBreakerRegistry which stores all CircuitBreaker
CircuitBreakerRegistry circuitBreakerRegistry = new InMemoryCircuitBreakerRegistry();

// Configure CircuitBreaker
// Configure the maximum number of allowed failures
// Configures the wait interval [ms] which specifies how long the CircuitBreaker should stay OPEN
...
The `map` function is only invoked, if the CircuitBreaker is CLOSED or HALF_CLOSED. `Try.of()` returns a `Success` Monad, if the invocation of the function was successful. If the function itself throws an exception, a `Failure` Monad is returned and the `map` function is not invoked.
...
In this test case the `map` function is not invoked, because the CircuitBreaker is OPEN. The call to `Try.of` returns a `Failure` Monad and `map` function is not invoked.
...
// Create a CircuitBreakerConfig to configure a CircuitBreaker
// Configure the maximum number of allowed failures
// Configures the wait interval [ms] which specifies how long the CircuitBreaker should stay OPEN
CircuitBreakerConfig circuitBreakerConfig = new CircuitBreakerConfig.Builder()
    .maxFailures(1).waitInterval(1000).build();
