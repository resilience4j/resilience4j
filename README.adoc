...
For example, if the size of the Ring Buffer is 10, then at least 10 calls must be evaluated, before the failure rate can be calculated. If only 9 calls have been evaluated the CircuitBreaker will not trip open even if all 9 calls have failed.
...
See http://javaslang.github.io/javaslang-circuitbreaker/0.3.0/[User Guide].
