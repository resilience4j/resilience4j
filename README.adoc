...
The CircuitBreaker is implemented via a (timed) state machine and has three states: `CLOSED`, `OPEN` and `HALF-OPEN`. The CircuitBreaker does not know anything about the backend's state by itself, but uses only the information provided by calls to `CircuitBreaker::recordSuccess()` and `CircuitBreaker::recordFailure()`.
