...
public class RetryAutoConfigurationTest {
	...
	public void testRetryAutoConfiguration() throws IOException {
		...
		// expect retry-event actuator endpoint recorded both events
		ResponseEntity<RetryEventsEndpointResponse> retryEventListA = restTemplate.getForEntity("/actuator/retryevents/retryBackendA", RetryEventsEndpointResponse.class);
		assertThat(retryEventListA.getBody().getRetryEvents()).hasSize(3);

		ResponseEntity<RetryEventsEndpointResponse> retryEventListB = restTemplate.getForEntity("/actuator/retryevents/retryBackendB", RetryEventsEndpointResponse.class);
