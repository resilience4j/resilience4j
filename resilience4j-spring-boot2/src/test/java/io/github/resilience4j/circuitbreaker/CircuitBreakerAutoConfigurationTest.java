...
public class CircuitBreakerAutoConfigurationTest {
	...
	@Autowired
	ReactiveDummyService reactiveDummyService;
	...
	public void testCircuitBreakerAutoConfiguration() throws IOException {
		...
		ResponseEntity<CircuitBreakerEventsEndpointResponse> circuitBreakerEventListBackEndA = restTemplate.getForEntity("/actuator/circuitbreaker-events/backendA", CircuitBreakerEventsEndpointResponse.class);
		assertThat(circuitBreakerEventListBackEndA.getBody().getCircuitBreakerEvents()).hasSize(2);
	...
	/**
	 * The test verifies that a CircuitBreaker instance is created and configured properly when the ReactiveDummyService(Flux and Mono types) is invoked and
	 * that the CircuitBreaker records successful and failed calls.
	 */
	@Test
	public void testCircuitBreakerAutoConfigurationReactive() throws IOException {
		...
		circuitBreakerEventList = restTemplate.getForEntity("/actuator/circuitbreaker-events?name=backendB", CircuitBreakerEventsEndpointResponse.class);
		assertThat(circuitBreakerEventList.getBody().getCircuitBreakerEvents()).hasSize(2);
