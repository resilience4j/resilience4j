...
public class CircuitBreakerAutoConfigurationRxJava2Test {
	...
	public void testCircuitBreakerAutoConfigurationReactiveRxJava2() throws IOException {
		...
		assertThat(healthResponse.getBody().getDetails().get("circuitBreaker").getDetails().get("backendA")).isNotNull();
		assertThat(healthResponse.getBody().getDetails().get("circuitBreaker").getDetails().get("backendB")).isNull();
